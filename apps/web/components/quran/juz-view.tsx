...
function JuzReadLink({ juzNumber }: { juzNumber: number }) {
  ...
  return (
    <Link
      ...
      className="inline-flex items-center gap-1.5 text-xs md:text-sm text-ink-soft hover:text-ink underline transition-colors duration-300"
    >
      {loading && (
        <span className="inline-block w-3 h-3 border-2 border-line border-t-gray-700 rounded-full animate-spin" />
      )}
    ...
    <section className={cn('w-full py-6 md:py-24 bg-paper')}>
      <Container>
        <div className="max-w-4xl mx-auto text-center mb-6 md:mb-16">
          ...
          <Text className="text-sm md:text-lg text-ink-soft max-w-2xl mx-auto leading-tight md:leading-relaxed">
            {searchQuery 
        ...
        {juzGroups.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8">
            {juzGroups.map((group) => (
              <div
                key={group.juzNumber}
                className="bg-surface border border-line rounded-lg p-3 md:p-6 shadow-sm hover:shadow-md transition-all duration-300 ease-in-out"
              >
                  ...
                  {group.surahs.map((surah) => {
                    ...
                    return (
                      <Link
                        ...
                        className="flex items-center gap-2 md:gap-4 p-2 md:p-3 rounded-lg hover:bg-surface-warm transition-all duration-300 ease-in-out border border-transparent hover:border-line"
                      >
                        <div className="flex-shrink-0 w-8 h-8 md:w-10 md:h-10 bg-line-soft rounded-md flex items-center justify-center">
                          <span className="text-ink font-bold text-xs md:text-sm">
                            {surah.surahNo}
                          </span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <div className="flex-1 min-w-0">
                              <Text className="text-ink font-semibold text-sm md:text-base">
                                {surah.surahNameTranslation}
                              </Text>
                              <Text className="text-ink-soft text-xs md:text-sm">
                                {surah.surahNameArabic}
                              </Text>
                            </div>
                          </div>
                          <Text className="text-ink-soft text-xs md:text-sm mt-0.5 md:mt-1">
